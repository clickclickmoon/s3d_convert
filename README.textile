...
bit mask.  The truth may never be known, but for now we just look for the known
good number and go with that.
...
The meta data for each file is in a twelve byte block and contains a 4 byte
...
_**file_size**_
The real size of the decomressed file.

h3. File Data Blocks

Each file is made up of one or more data blocks.  Each of these data blocks has
an eight byte header.  The file meta offset from above for the file points to
...
The very last file (in terms of offset into the file) is a zlib compressed file
that contains the list of file names in file offset order.  The first 4 bytes
are unknown, then there are 4 bytes of separator data (0, 13, 0, 0) then the
file name.  This directory file is not listed inside of itself.

h.4 Data Footer

Somtimes the s3d archive has a small signature on the bottom.  When there, it
reades "STEVEXXXX" where XXXX is the date.  This footer is ignored since it is
